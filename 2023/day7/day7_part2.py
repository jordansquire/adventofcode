...
def categorize_cards(cards) -> int:
    ...
    for char in cards:
        ...
        if char not in duplicate_map[repeat]:
            duplicate_map[repeat].append(char)

    if wild_count > 0:
        for i in range(4, 0, -1):
            if len(duplicate_map[i]):
                duplicate_map[i+wild_count].append(duplicate_map.get(i).pop())
                break

    if len(duplicate_map[5]) or wild_count == 5:
